        ...
        <div class="w-50 my-3 p-1 rounded shadow-lg">
            <div class="d-flex justify-content-between align-items-center">
                <a href="{{ route('profile.people', $data->user->id) }}"
                    ...
                    <span class="m-3 fs-5 text-dark">{{ $data->user->name }}</span>
                </a>
                <div class="d-flex align-items-center gap-2">
                    @if (Auth::user()->id == $data->user_id)
                        <svg data-bs-toggle="modal" data-bs-target="#editPhoto{{ $data->id }}"
                            ...
                            <path
                                d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.2931.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                    ...
                    @endif
                    <p class="text-muted fs-6 m-3">{{ date('d-m-Y', strtotime($data->created_at)) }}</p>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-between align-items-center">
                ...
                <form action="{{ route('photo.delete', $data->id) }}" method="POST">
                    @csrf
                    @method('delete')
                    <button type="submit" onclick="returnconfirm('Apakah anda yakin ingin menghapus?')"
                    ...
                    <li class="list-group-item border-start-0 border-end-0 d-flex flex-column">
                        <div class="d-flex justify-content-between align-items-center">
                            <a href="{{ route('profile.people', $comment->user->id) }}"
                                ...
                                <span class="ms-2 fs-5 text-dark">{{ $comment->user->name }}</span>
                            </a>
                            <div class="d-flex align-items-center">
                                @if (Auth::user()->id == $comment->user->id)
                                    ...
                                    <!-- Modal -->
                                    <div class="modal fade" id="editComment{{ $comment->id }}" tabindex="-1" aria-
                                    ...
                                    <form action="{{ route('comment.delete', $comment->id) }}" method="POST">
                                        ...
                                        @method('delete')
                                        <button class="btn"
                                            onclick="returnconfirm('Apakah anda yakin ingin menghapus komentar ini?')">
